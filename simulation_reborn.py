...
def go(lambd, mus, queue_lenghts, UI, drop_window = 100, isdynamic = False, discrepancy = 0.001, dynamic_window = 100,\
    ...
    while (process_is_not_finished):
        ...
        if (node_event == 0): # case generator
            ...
            if (node_event != lenmus): # case not last node
                ...
                current_innode_signal_index = updateLastNode(data, t, node_event, intensivities, queue_lenghts, \
                                                             node_states, node_queue_states, upcoming_event_times, current_innode_signal_index)
        ...
        if (iterator % drop_window == 1):
            ...
            if (dynamic == False):
                process_is_not_finished = processIsNotFinishedStatic(finished, number_of_samples,UI)
        
        if (dynamic == True):
            if (iterator % dynamic_window == 1):
                stationary, last_probabilities = isStationary(node_state_dataframe, last_probabilities,\
                                                              discrepancy, dynamic_window, lenmus, UI, iterator)
                if (stationary):
                    dynamic = False
    ...
    UI.Finaly(iterator, simulation_duration, folder_path)
...
def isStationary(node_state_dataframe, last_probabilities, discrepancy, dynamic_window, lenmus, UI, iterator):
    ...
    UI.UI_DM(stationary_list_per_node, iterator)
...
def getProbabilities(node_state_dataframe, dynamic_window, last_probabilities, lenmus):
    ...
    for temp_arr_iter in last_probabilities:
        new_probabilities.append(temp_arr_iter.copy()) # Must use this, otherwise changing new_probabilities affects last_probabilities
...
def processIsNotFinishedStatic(finished, number_of_samples,UI):
    l = len(finished)
    #label = Label()
    if (l >= number_of_samples):
        UI.UI_ST(Actual_Progress=str(100))
        return False
    else:
        # UI INTEGRATION
        UI.UI_ST(Actual_Progress=str(round(l*100/number_of_samples, 2)))
        return True
...
def writeSimulation(final, unstaionary, node_state_dataframe, arrival_rate, array_of_departure_rates,\
    ...
    with open(directory_path + "/configuration.txt", "w+") as text_file:
        configuration_txt = makeConfigurationTXT(arrival_rate, array_of_departure_rates, array_of_queue_lengths,\
                                                 finish_drop_window, dynamic, discrepancy, dynamic_window, number_of_samples,\
...
def makePath():
    return 'RUN_' + time.strftime("%d_%m_%Y_%H_%M_%S")


def makeConfigurationTXT(lambd, mus, queue_lengths, drop_window, dynamic, discrepancy, dynamic_window, number_of_samples,\
                         dynamic_after_stationary_number_of_samples):
    string = 'Simulation Configuration\n\n'
    string = string + 'Arrival Rate: ' + str(lambd) + '\n'
    lenmus = len(mus)
    for i in range(lenmus):
        string = string + 'Node ' + str(i) + ' departure rate: ' + str(mus[i]) + ', max queue length: ' + str(queue_lengths[i]) + '\n'
    if (dynamic == True):
